...
include_once("../../../asset/inc/config.php");
  ...
  if($p_pad != "") {
    ...
    $x = explode('.', $p_pad); //memisahkan nama file dengan ekstensi yang diupload
    $ekstensi = strtolower(end($x));
    $file_p_pad_tmp = $_FILES['p_pad']['tmp_name']; 
    $acak = rand(00000000, 99999999);
    $p_padExt = substr($p_pad, strrpos($p_pad, '.')); 
    $p_padExt = str_replace('.','',$p_padExt); // Extension
    $p_pad = preg_replace("/\.[^.\s]{3,4}$/", "", $p_pad); 
    $nama_p_pad_baru = "PAD_".$acak.'.'.$p_padExt;
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
      move_uploaded_file($file_p_pad_tmp, 'file/pad/'.$nama_p_pad_baru); //memindah file ke folder file/p_dd
    }
  }
  ...
  if($foto4 != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
      move_uploaded_file($file_foto4_tmp, 'file/foto/foto4/'.$nama_foto4_baru); //memindah file ke folder file/foto1
        ...
        $result = mysqli_query($koneksi, "INSERT INTO tb_realisasi_pad(realisasi_pad,foto_pad_1,foto_pad_2,foto_pad_3,foto_pad_4,id_user,validasi,tahun) VALUES('$nama_p_pad_baru','$nama_foto1_baru','$nama_foto2_baru','$nama_foto3_baru','$nama_foto4_baru','$id_user','$valid','$tahun')");
