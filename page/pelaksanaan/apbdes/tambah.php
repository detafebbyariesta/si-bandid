...
include_once("../../../asset/inc/config.php");
  ...
  if($p_apbdes != "") {
    ...
    $x = explode('.', $p_apbdes); //memisahkan nama file dengan ekstensi yang diupload
    $ekstensi = strtolower(end($x));
    $file_p_apbdes_tmp = $_FILES['p_apbdes']['tmp_name']; 
    $acak = rand(00000000, 99999999);
    $p_apbdesExt = substr($p_apbdes, strrpos($p_apbdes, '.')); 
    $p_apbdesExt = str_replace('.','',$p_apbdesExt); // Extension
    $p_apbdes = preg_replace("/\.[^.\s]{3,4}$/", "", $p_apbdes); 
    $nama_p_apbdes_baru = "APBDES_".$acak.'.'.$p_apbdesExt;
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
      move_uploaded_file($file_p_apbdes_tmp, 'file/apbdes/'.$nama_p_apbdes_baru); //memindah file ke folder file/p_dd
    }
  }
  ...
  if($foto4 != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
      move_uploaded_file($file_foto4_tmp, 'file/foto/foto4/'.$nama_foto4_baru); //memindah file ke folder file/foto1
        ...
        $result = mysqli_query($koneksi, "INSERT INTO tb_realisasi_apbdes(realisasi_apbdes,foto_apbdes_1,foto_apbdes_2,foto_apbdes_3,foto_apbdes_4,id_user,validasi,tahun) VALUES('$nama_p_apbdes_baru','$nama_foto1_baru','$nama_foto2_baru','$nama_foto3_baru','$nama_foto4_baru','$id_user','$valid','$tahun')");
